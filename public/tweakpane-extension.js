...
(function () {
    window.enableButtonRename = function (id) {
        const button 	= THOTH.FE.layerButtons.get(id);
        const layer 	= THOTH.Scene.currData.layers[id];
        const attr 		= "name";

        const buttonElement = button.controller.buttonController.view.buttonElement; // at ButtonView a div element is created and inside it a button named tp-btn_b ->buttonElement
        ...
        titleElem.addEventListener('dblclick', () => {
            ...
            const applyRename = () => {
                ...
                if (newTitle !== '') {
					...
					const oldTitle = layer["name"];

					// Add to history
					THOTH.HIS.pushAction(
						THOTH.HIS.ACTIONS.RENAME_LAYER,
						id,
						{
							"newTitle": newTitle,
							"oldTitle": oldTitle
                ...
                THOTH._bListenKeyboardEvents = true;
            };
